...
const PROVIDERS = {
  free: { label: '刷亞澤的信用卡(免費體驗,每日限量)', base: 'https://lcm-ai-proxy.yazelinj303.workers.dev', model: 'openai/gpt-oss-120b', writerModel: 'glm-5.2', keyless: true }, // 執行=Groq gpt-oss;編劇=朋友的 GLM(worker 依 model 路由)。代理見 repo worker/
...
const TOOL_DEFS = [
  ...
  { name: 'apply_script', description: "修改腳本並立即更新畫面:settings 淺合併、people 整組取代、messages 整列取代;三個欄位都可省略,只給要改的。每則訊息一定要有 type 欄位(不是 content/person 那種自創欄位),否則會被丟掉。", parameters: { type: 'object', properties: { settings: { type: 'object', description: "只放要改的欄位,例 { title: '群組名稱' }" }, people: { type: 'array', items: { type: 'object', required: ['id', 'name'], properties: { id: { type: 'string', description: 'messages[].personId 會引用這個值' }, name: { type: 'string' }, avatar: { type: ['string', 'null'], description: "沒有就給 null。你產不出真的圖片:自己編的 base64 解碼後 IDAT 會斷,瀏覽器載不出來。外部網址也會被忽略。要頭像請叫使用者自己上傳,或用站內的「AI 補圖」生成" } } } }, messages: { type: 'array', items: { type: 'object', required: ['type'], properties: { type: { type: 'string', enum: ['msg', 'date', 'skip'], description: "msg=一則訊息;date=日期分隔線;skip=「⋯⋯(略)⋯⋯」省略記號" }, kind: { type: 'string', enum: ['text', 'image', 'sticker', 'voice', 'file'], description: '只有 type=msg 用,預設 text' }, side: { type: 'string', enum: ['left', 'right'], description: "right=自己,綠泡泡,不要給 personId;left=別人,要給 personId" }, text: { type: 'string', description: 'type=msg 且 kind=text 時是訊息內容;type=date 時是日期字串,例「7月17日 (四)」' }, personId: { type: 'string', description: '只有 side=left 要給,值必須對得上 people[].id' }, time: { type: 'string', description: "顯示時間,例「下午4:06」" }, read: { type: 'string', description: "只有 side=right 有意義,例「已讀」或「已讀 8」" }, imgDesc: { type: 'string', description: "kind=image/sticker 必填:那張圖畫的是什麼(照劇本的[圖片:描述]寫)。AI 補圖完全靠這個欄位,漏了就會畫出跟劇情無關的東西" }, dur: { type: 'string', description: "kind=voice 用,語音長度,例「0:12」" }, fname: { type: 'string', description: 'kind=file 用,檔名,例「企劃書.pdf」' }, fsize: { type: 'string', description: "kind=file 用,檔案大小,例「2.4MB」" } } } } }, additionalProperties: false } },
  { name: 'append_messages', description: '在對話尾端加入訊息,畫面立即更新。每則一定要有 type 欄位。', parameters: { type: 'object', properties: { messages: { type: 'array', items: { type: 'object', required: ['type'], properties: { type: { type: 'string', enum: ['msg', 'date', 'skip'], description: "msg=一則訊息;date=日期分隔線;skip=「⋯⋯(略)⋯⋯」省略記號" }, kind: { type: 'string', enum: ['text', 'image', 'sticker', 'voice', 'file'], description: '只有 type=msg 用,預設 text' }, side: { type: 'string', enum: ['left', 'right'], description: "right=自己,綠泡泡,不要給 personId;left=別人,要給 personId" }, text: { type: 'string', description: 'type=msg 且 kind=text 時是訊息內容;type=date 時是日期字串,例「7月17日 (四)」' }, personId: { type: 'string', description: '只有 side=left 要給,值必須對得上 people[].id' }, time: { type: 'string', description: "顯示時間,例「下午4:06」" }, read: { type: 'string', description: "只有 side=right 有意義,例「已讀」或「已讀 8」" }, imgDesc: { type: 'string', description: "kind=image/sticker 必填:那張圖畫的是什麼(照劇本的[圖片:描述]寫)。AI 補圖完全靠這個欄位,漏了就會畫出跟劇情無關的東西" }, dur: { type: 'string', description: "kind=voice 用,語音長度,例「0:12」" }, fname: { type: 'string', description: 'kind=file 用,檔名,例「企劃書.pdf」' }, fsize: { type: 'string', description: "kind=file 用,檔案大小,例「2.4MB」" } } } } }, required: ['messages'], additionalProperties: false } },
...
// 真的把 data: 圖畫出來看有沒有東西。模型很愛編 base64:標頭抄得很像,IDAT 卻是斷的。
// 不能只看 onload 或 naturalWidth —— Chrome 對截斷的 PNG 很寬容,IHDR 給了尺寸就報成功,
// 但畫出來整片透明,視覺上等於沒有頭像。所以要畫進 canvas 數不透明像素。
function imgLoads(src) {
  return new Promise((resolve) => {
    const im = new Image();
    im.onerror = () => resolve(false);
    im.onload = () => {
      if (!im.naturalWidth) return resolve(false);
      try {
        const n = 16; // 縮成 16x16 取樣就夠判斷「整片透明」,不必逐像素掃原圖
        const c = document.createElement('canvas'); c.width = n; c.height = n;
        const g = c.getContext('2d');
        g.drawImage(im, 0, 0, n, n);
        const d = g.getImageData(0, 0, n, n).data;
        for (let i = 3; i < d.length; i += 4) if (d[i] > 8) return resolve(true);
        resolve(false); // 全透明 = 沒有內容
...
- settings.draft 只放「自己」還沒送出的話。輸入框是手機主人的,看不到對方在打什麼。劇本若把草稿寫成左側角色的台詞,那是劇本錯了:把它改成該角色的一般訊息,或整個不要,**不要照抄進 draft**。
- 時間照台灣 LINE 慣例如「下午4:06」,前後訊息時間要合理遞增。連續同 personId 的 left 訊息會自動省略頭像暱稱。
- **每份對話一定要有「自己」(至少一位角色是 side:"right" 綠泡泡)**:劇本若指明誰是自己/我方(角色設定標了「(自己)」或用「我」),就把他的台詞全設 side:"right"(不給 personId、也不列進 people);若劇本沒指明(三個人都只有名字、沒有「我」),你要挑一位最適合當截圖者的角色當自己(通常是被鬧/負責吐槽/主角那位),其台詞設 side:"right",其餘才是 left。絕不可讓所有人都排在左邊、畫面一顆綠泡泡都沒有——那不像真的 LINE 截圖。群組 members 含自己一起算。
...
**被討論的視覺物件一定要現形**:只要劇情裡有人「傳一張圖/畫了什麼/曬截圖/秀某張照片/看某個東西」,那個東西**本身**就要用「[圖片:描述]」或「[貼圖:描述]」做成一則訊息讓觀眾看到;絕不能只用文字說「那張圖」「一坨黑黑的東西」而畫面上根本沒有那張圖——那是 tell 不是 show,梗會落空。劇情的核心物件(畫作、截圖、照片、梗圖、成績單、菜單…)沒現形 = 犯規。
...
- 視角人物(誰是「自己」):不論群組或雙人,都要指定一位角色是「自己」——拿手機截圖的人,他的台詞在右側綠泡泡。在開頭角色設定裡於該角色名後標「(自己)」,執行端才知道誰排右邊。少了這個,畫面會變成所有人都擠在左邊、一顆綠泡泡都沒有,不像真的 LINE。挑最適合當截圖者的那位:通常是被鬧的、負責吐槽的、或故事主角。他不會出現在左側名單,名字也不顯示(因為那就是「你」)。
- 輸入框草稿:「(草稿) draft=打了沒送出的那句話」→ 顯示在輸入框;強力的懸念收尾,但屬於選配——不是每部都要用,連用會膩。
  **草稿一定是「自己」的,絕不可寫成對方的。** 輸入框是拿手機那個人的,你看不到對方正在打什麼字,LINE 沒有那種功能。
  寫成對方的草稿,畫面上就會出現一句不可能被看到的話,而且它會被排進自己的輸入框,讀者一眼穿幫。
  想表現對方欲言又止,改用別的手法:已讀不回、時間拉開、對方只回一個貼圖
...
6) settings.draft 若有值,那句話是「自己」要說的嗎?若是左側角色的台詞,改成一般訊息或清空 draft
7) settings 的 title/mode 與內容相符?
...
async function writeScreenplay(prompt, existing) { // 編劇→評審迴圈,及格(或 3 輪取最佳)才放行
  ...
  const wcfg = explicitW || cfg(); // 評審(打分)走這個
  // 編劇(創意):沒另設編劇時,若主 provider 有指定的編劇模型就用它(免費→GLM、Gemini→pro);評審維持執行模型省額度
  const writerModel = (!explicitW && PROVIDERS[wcfg.provider] && PROVIDERS[wcfg.provider].writerModel) || wcfg.model;
  const wuse = { ...wcfg, model: writerModel };
  log(`編劇使用:${PROVIDERS[wcfg.provider] ? PROVIDERS[wcfg.provider].label : wcfg.provider} · ${writerModel}`);
  const brief = writerBrief(prompt, existing);
  const wmsgs = [{ role: 'system', content: WRITER_SYSTEM }, { role: 'user', content: brief }];
  let best = { script: '', total: -1 };
  for (let round = 1; round <= 3; round++) {
    const script = textOf(await chat(wmsgs, false, true, wuse));
    if (!script) break;
...
- 每一格都必須在句首自己寫明畫風,不可省略:所有格會畫在同一張網格圖上、彼此畫風會互相感染,只有每格都把畫風標死,寫實的頭像才不會被卡通貼圖帶成動漫風。
- 同一人物在不同格外觀必須一致:先自行設定(髮型/衣著/年齡/體型),每個出現該人物的格都重複同一套描述
- 貼圖格:「Q版可愛貼圖插畫風」,主體置中佔滿,背景整片純綠色,無文字無邊框
- 照片格:「寫實手機隨手拍照片風」,真實質感,自然光,構圖填滿整格
- 頭像格:先依這個人的個性與劇情,選一種最搭他的畫風(寫實人像照/清新插畫頭像/卡通角色/他養的寵物/一個代表物…),在句首把選定的畫風標死;不同人本來就該不同風,重點是「每個頭像各自選定並寫明畫風」,別讓整盤被某一格帶成同一種風。主體置中、單色淺色背景
...
$('#ai-copy-prompt').addEventListener('click', async () => { // 複製劇本 prompt,拿去外部 GPT/Gemini/Claude 產生,再貼回劇本欄
  const prompt = $('#ai-prompt').value.trim();
  const existing = $('#ai-screenplay-text').value.trim();
  if (!prompt && !existing) { $('#ai-prompt').focus(); toast('先在「劇情主題」寫幾個字,或劇本欄先起個頭'); return; }
  const full = WRITER_SYSTEM + '\n\n─────── 以下是你的委託 ───────\n' + writerBrief(prompt || '把既有劇本整體強化', existing) +
    '\n\n請直接輸出一份完整劇本(不要多餘說明);產生後把整段貼回製造機的「劇本」欄,再按「開始製作」。';
  try { await navigator.clipboard.writeText(full); toast('已複製劇本 prompt;貼到 ChatGPT／Gemini／Claude 產生後,把劇本貼回下方「劇本」欄'); }
  catch (e) { $('#ai-screenplay-text').value = full; screenplaySave(); toast('無法自動複製;已把 prompt 放進劇本欄,請自行剪下貼到外部 AI(記得產生後貼回)'); }
});
